...
void HWBridge::outputEmptyPoint(unsigned int deviceNumber) {
	ISPDB25Point point;
	point.x = 0x8000;
	point.y = 0x8000;
	point.r = 0x0000;
	...
	this->device->writeFrame(emptySlice, emptySlice.durationUs, deviceNumber);
}
...
void HWBridge::driverLoop() {
	...
	std::vector<std::shared_ptr<SliceBuf>> currentBufs;
	...
	while (true) {
		...
		for (int i = 0; i < bexes.size(); i++)
		{
			if (currentBufs.size() <= i)
				currentBufs.push_back(std::make_shared<SliceBuf>());

			std::shared_ptr<BEX> bex = bexes[i];
			...
			if (newBuf) {
				...
				currentBufs[i] = newBuf;

				//only trim and adjust speed in wave mode
				if (bex->getMode() == DRIVER_WAVEMODE) {
					speedFactor = calculateSpeedfactor(speedFactor, currentBufs[i]);
				}
				else if (bex->getMode() == DRIVER_FRAMEMODE) {
					speedFactor = 1.0;
				}
			...
			unsigned currentBufSize = currentBufs[i]->size();
			for (int i = 0; i < currentBufSize; i++) {
				clock_gettime(CLOCK_MONOTONIC, &then);
				std::shared_ptr<TimeSlice> nextSlice = currentBufs[i]->front();
				currentBufs[i]->pop_front();

				//if we're in frame mode, put the slice back
				//wave mode just discards
				if (bex->getMode() == DRIVER_FRAMEMODE) {
					currentBufs[i]->push_back(nextSlice);
				}

				this->device->writeFrame(*nextSlice, speedFactor * nextSlice->durationUs, i);
...
double HWBridge::calculateSpeedfactor(double currentSpeed, std::shared_ptr<SliceBuf> buffer) {
	...
	if (buffer->size() != 0) {
		...
		double newSpeed = (1.0 + offCenter + 0.05 * accumOC);
