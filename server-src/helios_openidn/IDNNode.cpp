...
IDNNode::IDNNode(std::shared_ptr<HWBridge> hwBridge) {
...
void IDNNode::commitChunk(unsigned int deviceNumber) {
	if (currentSlicePoints.size() != 0) {
		...
		driverPtr->getBEX(deviceNumber)->networkAppendSlice(newSlice);
		resetChunkBuffer();
	}
}
...
int IDNNode::processIDNPacket(char* buf, unsigned int len, int sd, struct sockaddr* remote, unsigned int addr_len) {
	...
	******************** */
		...
		char dacName[20] = { 0 };
		driverPtr->getDevice()->getName(dacName);
		if (dacName[0] == '\0')
		...
		*/
		//Compare Channel ID
		if (((channel->remote.sa_family == remote->sa_family)
				&& (strncmp(channel->remote.sa_data, remote->sa_data, 14) == 0)
				&& ((chMsgHeader->cnl & IDN_CH_MSG_HEADER_CNL_CH_BMASK) == channel->chId)) || !(channel->valid)) {
